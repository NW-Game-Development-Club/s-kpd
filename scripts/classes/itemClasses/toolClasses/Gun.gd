...
func onActivate():
	if weaponDetectionType == DETECTION_TYPE.HIT_SCAN:
		...
		if result:
			...
			return 0
	elif weaponDetectionType == DETECTION_TYPE.PROJECTILE:
		var bullet:Bullet = projectilePrefab.instantiate()
		crossHair.get_tree().root.add_child(bullet)
		bullet.global_transform = firePoint.global_transform
		bullet.linear_velocity = -firePoint.global_transform.basis.z * bullet.bulletSpeed
